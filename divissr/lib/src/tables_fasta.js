...
for (let rep in allRepClasses) {
    ...
    summaryTableData.push({
        ...
        'freqPercent': ((repeatClassAbundanceData[rep][1]/data.info.RepInfo.TotalRepFreq)*100).toFixed(2),
        'bases': repeatClassAbundanceData[rep][0],
        'basesPercent': ((repeatClassAbundanceData[rep][0]/data.info.RepInfo.TotalRepBases)*100).toFixed(2)
    })
}

const summaryColumns = [ 'Repeat Class', 'Frequency', '% Frequency', 'Bases', '% Bases' ];
const repeatLocColumns = ['Sequence id', 'Start', 'Stop', 'Repeat Class', 'Repeat length', 'Strand', 'Units', 'Actual Repeat'];
const summaryColKeys = ["repClass", "freq", "freqPercent", "bases", "basesPercent"];
