...
const repeatData = data.info.RepInfo.PlotData;
const plotData = {};
Object.keys(repeatData).forEach(r => { plotData[r] = repeatData[r]['LenFreq'] })
...
const dataTypes = ['Bases', 'Frequency', 'Reads']
let current_chart = 0;

let globalMinLength = data.info.RepInfo.MinLength;
let globalRepeatMaxLength = data.info.RepInfo.MaxLength;
allRepClasses.forEach( e => {
    const frequency = repeatData[e]['Freq'];
    const bases = repeatData[e]['Bases'];
    const reads = repeatData[e]['Reads'];
    repeatClassAbundanceData[e] = [bases, frequency, reads];
})
...
const sortedRepeatClassesByReads = data.info.RepInfo.AllRepClasses.map(x => x);
sortedRepeatClassesByReads.sort((a,b) => { return repeatClassAbundanceData[a][2] - repeatClassAbundanceData[b][2] });
...
allRepClasses.forEach(a => { 
    ...
    allKmersAbundanceData[kIndex][2] += repeatClassAbundanceData[a][2]; 
});
...
const plotRepeatAbundance = function() {
    ...
    const dataIndex = dataTypes.indexOf(parameters.repeatAbundanceDataType);
    ...
    const repClassLengthRangeAbundance = selectedRepeats.map( a =>  {
        ...
        allLengths.forEach( l => { 
            if ( l <= parameters.plotMaxLength && l >= parameters.plotMinLength) {
                if (dataIndex === 1) { total += plotData[a][l]  }
                else { total += (plotData[a][l] * l); }
            }
        })
        return total; 
    })
    ...
    return selectedRepeats.map( a => {
        return {
            ...
            data: plotLengths.map(l => {
                if (l in plotData[a]) { return plotData[a][l] }
                else { return 0; }
            })
        }
    })
    ...
    selectedRepeats.forEach( a => {
        ...
        series.push({
            ...
            xAxisIndex: 0, yAxisIndex: 0, label: {
                ...
                formatter: function(d) {
                    const di = dataTypes.indexOf(parameters.repeatAbundanceDataType);
                    if (d.data !== 0) { return d.seriesName; } else { return ''; }
                }
            }
        });
    })
    const total = (dataIndex === 0) ? parseInt(data.info.RepInfo.RepeatBases) : parseInt(data.info.RepInfo.TotalRepeats);
    kmers.forEach((k,i) => { 
...
// Static Chart of ReadLengthDistribution
const readLenChart = echarts.init(document.getElementById('read-len-chart'));
const readlenData = data.info.SeqInfo.ReadLengthDist;
const allreadLens = Object.keys(readlenData).map(d => { return parseInt(d) })
allreadLens.sort((a,b) => { return a-b; })
let readLenSeries = [];
allreadLens.forEach((k,i) => { readLenSeries.push({
    type: 'bar', name: k, stack: 'Distribution', data:[parseInt(readlenData[k])],
    label: {show: true, position: 'inside', formatter: function(d) { return `${((d.data/data.info.SeqInfo.TotalReads)*100).toFixed(2)} %`; }}
}) })
const readLenChartOptions = {
    textStyle: { fontFamily: 'Ubuntu Mono',},
    grid: { containLabel: true, bottom: 20, top: 40, left: 60, right: 60 },
    title: { text: 'Read Length Distribution', left: 'center' },
    tooltip: {
        trigger: 'axis',
        axisPointer: { type: 'shadow' },
    },
    yAxis: { data: ['Distribution'], axisTick: { show: true, interval: 0 }, axisLabel: { show: true, interval: 0, rotate: 0 } },
    xAxis: { max: data.info.SeqInfo.TotalReads, min: 0, axisTick: { show: true }, axisLabel: { show: true } },
    series: readLenSeries,
    toolbox: {
        right: 20,
        feature: {
          saveAsImage: {  title: 'Save as PNG', show: true,  type: 'png',  name: `${data.info.SeqInfo.FileName}_ReadLengthDistribution` 
          },
          dataView: {  title: 'View data',  readOnly: true,  lang: ['Data View', 'Go  back'] 
          },
        }
...
readLenChart.setOption(readLenChartOptions);
window.onresize = function() { readLenChart.resize(); mainChart.resize(); }
...
allKmers.forEach((k,i) => { allkmerSeries.push({
    ...
    label: {show: true, position: 'inside', formatter: function(d) { return `${((d.data/data.info.RepInfo.TotalRepeats)*100).toFixed(2)} %`; }}
}) })
const kmerChartOptions = {
    ...
    yAxis: { data: ['Distribution'], axisTick: { show: true, interval: 0 }, axisLabel: { show: true, interval: 0, rotate: 0 } },
    xAxis: { max: data.info.RepInfo.TotalRepeats, min: 0, axisTick: { show: true }, axisLabel: { show: true } },
